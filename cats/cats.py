...
def pick(paragraphs, select, k):
    ...
    for i in range (0, len(paragraphs)):
        select_check = select(paragraphs[i])
        if select_check is True:
            check_lst.append(paragraphs[i])

    if k <= ((len(check_lst)) - 1):
        return check_lst[k]
    else:
        return ''
    # END PROBLEM 1
...
def wpm(typed, elapsed):
    ...
    min_to_sec = 60 / elapsed
    average = word_count * min_to_sec
    return average
    # END PROBLEM 4
...
def autocorrect(typed_word, word_list, diff_function, limit):
    ...
    if typed_word in word_list:
        return typed_word
    lst = [diff_function(typed_word, words, limit) for words in word_list]
    if min(lst) > limit:
        return typed_word
    else:
        closest_word = word_list[lst.index(min(lst))]
        return closest_word
...
def feline_fixes(typed, source, limit):
    ...
    if len(typed) == 0 or len(source) == 0:
        limit -= max(len(source), len(typed))
...
def report_progress(typed, prompt, user_id, upload):
    ...
    for i in range (len(typed)):
        if typed[i] == prompt[i]:
            ...
            break
    progress = wrd_correct/len(prompt)
    upload({'id' : user_id, 'progress': progress })
    return progress
    # 1. calculate progress = # correctly typed words / # words in the prompt
    # 2. upload()
    # 3. return progress
    # END PROBLEM 8
...
def time_per_word(words, times_per_player):
    ...
    # BEGIN PROBLEM 9
    wrd_lst = []
    h = len(times_per_player)
    for p in range(h):
        wrd_lst0 = []
        for t in range(len(times_per_player[p])):
            if t != len(times_per_player[p]) - 1:
                wrd_lst0.append(times_per_player[p][t+1]-times_per_player[p][t])
        wrd_lst.append(wrd_lst0)
    return match(words, wrd_lst)
    # END PROBLEM 9
...
def fastest_words(match):
    ...
    # BEGIN PROBLEM 10
    fast_time = []
    for wrd in word_indices:
        p0 = []
        for plyr in player_indices:
            p0.append(time(match, plyr, wrd))
        fast_time.append(p0)
    p = []
    for t in fast_time:
        p.append(t.index(min(t)))
    f = []
    for t in player_indices:
        f.append([])
    for m in range(len(p)):
        l = p[m]
        f[l] = f[l] + [get_word(match, m)]
    return f
